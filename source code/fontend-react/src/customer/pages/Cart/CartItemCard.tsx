...
const CartItemCard : React.FC<CartItemProps> = ({ item }) => {
    ...
    // Return empty div if product data is missing
    if (!item || !item.product) {
        return <div className="border rounded-md p-5">Product data not available</div>;
    }
        ...
        <div className='border rounded-md relative'>
            <div className='p-5 flex gap-3'>
                ...
                <div className='space-y-2'>
                    <h1 className='font-semibold text-lg'>{item.product?.seller?.businessDetails?.businessName || 'Unknown Seller'}</h1>
                    <p className='text-gray-600 font-medium text-sm'>{item.product.title}</p>
                    <p className='text-gray-400 text-xs'><strong>Sold by:</strong> {item.product?.seller?.businessDetails?.businessName || 'Unknown Seller'}</p>
